...
# Paths. VERL_ROOT pins us to the verl repo regardless of where this script
# physically lives (e.g. mirrored copy under SF-UIDA-main/scripts/rl/).
VERL_ROOT="${VERL_ROOT:-/home/luoxuan/runcodes/verl}"
ACTOR_PATH="${ACTOR_PATH:-$VERL_ROOT/checkpoints/sft_hotpot_cot_hf/global_step_500}"
...
REWARD_PY="${REWARD_PY:-$VERL_ROOT/src/rewards/hotpot.py}"

DATE_TAG="$(date +%Y%m%d)"
RUN_NAME="phase3_${REWARD_VERSION}_a${ALPHA}_s${SEED}_${DATE_TAG}"
[[ "$SMOKE" == "1" ]] && RUN_NAME="${RUN_NAME}_smoke"

OUTPUT_DIR="${OUTPUT_DIR:-$VERL_ROOT/checkpoints/$RUN_NAME}"
